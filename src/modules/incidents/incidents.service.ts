...
export const incidentsService = {
  ...
  async getIncidentById(id: string): Promise<any> {
    const response = await api.get(API_ROUTES.incidents.getById(id));
    return response.data;
  },

  async assignOperator(incidentId: string, operatorId: string): Promise<any> {
    const response = await api.patch(API_ROUTES.incidents.assignOperator(incidentId), {
      operatorId,
    });
    return response.data;
  },
